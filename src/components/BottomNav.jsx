...
const navItems = [
  ...
  { to: '/scanner', icon: Camera, label: 'IA', special: true },
...
export default function BottomNav() {
  return (
    <nav className="fixed bottom-0 left-0 right-0 z-40 bg-navy-800/95 backdrop-blur-lg border-t border-navy-700/50">
      <div className="max-w-2xl mx-auto flex items-center justify-around px-1 py-2">
        {navItems.map((item) => {
          const Icon = item.icon
          if (item.special) {
            return (
              <NavLink key={item.to} to={item.to} className="flex flex-col items-center -mt-7">
                ...
                <span className="text-[10px] font-semibold text-chantier mt-1">{item.label}</span>
              </NavLink>
            )
          }
          return (
            <NavLink
              ...
              to={item.to}
              className={({ isActive }) =>
                `flex flex-col items-center gap-1 px-2 py-1.5 rounded-xl transition-colors min-w-[60px] ${
                  isActive ? 'text-chantier' : 'text-gray-400'
                }`
              }
            >
              {({ isActive }) => (
