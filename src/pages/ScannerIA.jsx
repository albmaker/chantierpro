...
// Simulation réaliste de fallback si l'API ne répond pas
async function simulateAIAnalysis() {
  ...
  const suggestions = [
    ...
    },
    {
      metier: 'electricite',
      confidence: 88,
      description: "Installation électrique à mettre aux normes : prises et points lumineux.",
      ouvrages: [
    ...
    },
    {
      metier: 'peinture',
      confidence: 95,
      description: "Travaux de peinture intérieure sur ~45m² (murs + plafonds).",
      ouvrages: [
    ...
    },
    {
      metier: 'maconnerie',
      confidence: 90,
      description: "Travaux de cloison et carrelage sol sur ~30m².",
      ouvrages: [
...
export default function ScannerIA() {
  ...
  async function analyzeImage() {
    ...
    try {
      // Tente d'abord la vraie API Mistral
      const result = await analyzeChantierImage(image)
      setResult(result)
      setApiMode('real')
    } catch (err) {
      console.warn('Mistral API non disponible, fallback simulation:', err.message)
  ...
  return (
    <div className="pb-24">
      <Header title="📸 Scanner Chantier IA" subtitle="Photo → Devis auto" />
          ...
          <>
            <div className="card bg-gradient-to-r from-purple-500/10 to-chantier/10 border border-purple-500/20">
              <div className="flex items-center gap-2">
                ...
                <div>
                  <p className="text-sm font-semibold text-white">Propulsé par Mistral AI</p>
                  <p className="text-xs text-gray-300">IA française · Vision par ordinateur</p>
                </div>
              </div>
            </div>
            ...
            >
              ...
              <p className="text-sm text-gray-300 mb-4 px-4">Notre IA Mistral analyse et génère un devis estimatif</p>
          ...
          <>
            <div className="card p-0 overflow-hidden">
              ...
              <div className="p-3 flex justify-between items-center">
                <span className="text-sm text-gray-300">Photo du chantier</span>
            ...
            {analyzing && (
              <div className="card text-center py-8">
                ...
                <h3 className="text-lg font-semibold text-white mb-1">L'IA analyse votre chantier...</h3>
                <p className="text-sm text-gray-400">Identification des ouvrages (~3-5s)</p>
              </div>
            )}
              ...
              <div className="space-y-3 animate-slide-up">
                <div className="card border border-chantier/30 bg-gradient-to-br from-chantier/10 to-transparent">
                  <div className="flex items-center gap-2 mb-2 flex-wrap">
                    ...
                    {apiMode === 'simulated' && (
                      <span className="badge bg-yellow-500/20 text-yellow-400 text-[10px]">Mode démo</span>
                    )}
                  </div>
                  <p className="text-sm text-gray-300 mb-3">{result.description}</p>
                  <div className="flex items-center gap-2 flex-wrap">
                    <span className="badge bg-chantier/20 text-chantier">Confiance : {result.confidence}%</span>
                    <span className="badge bg-navy-600 text-gray-300 capitalize">{result.metier}</span>
                  </div>
                </div>
                    ...
                    {result.ouvrages.map((o, i) => (
                      <div key={i} className="flex items-center justify-between p-2 bg-navy-800/50 rounded-lg">
                        <div>
                          ...
                          <p className="text-xs text-gray-400">{o.qty} {o.unit} × {o.priceHT}€ HT · TVA {o.tva}%</p>
                        </div>
